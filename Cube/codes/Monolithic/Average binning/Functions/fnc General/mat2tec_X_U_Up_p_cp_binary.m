function [] = mat2tec_X_U_Up_p_cp_binary(X, U, U_std, p, cp, varargin)
    ...
    %   Restructure Data
    tdata=[];
    tdata.Nvar=19; %number of variables
    ...
    tdata.varnames={'X','Y','Z',...                                         % 3
                    ...
                    'p [Pa]','c_p [-]',...                                  % 3 (8-9)
                    'Om_x [1/s]','Om_y [1/s]','Om_z [1/s]','|Om| [1/s]',... % 4 (10-13)
                    'Q','np','isvalid'};                                    % 3 (14-16)   
    ...
    % Pressure, p + cp
    tdata.cubes(1).v(8,:,:,: )=p;
    tdata.cubes(1).v(9,:,:,: )=cp;
    % Vorticity, Omega
    tdata.cubes(1).v(10,:,:,: )=omega_x;
    tdata.cubes(1).v(11,:,:,: )=omega_y;
    tdata.cubes(1).v(12,:,:,: )=omega_z;
    tdata.cubes(1).v(13,:,:,: )=OMEGA;
    % Q-criterion + flag variables
    tdata.cubes(1).v(14,:,:,: )=Q;
    tdata.cubes(1).v(15,:,:,: )=U{4};%np
    tdata.cubes(1).v(16,:,:,: )=isvalid2;%np
