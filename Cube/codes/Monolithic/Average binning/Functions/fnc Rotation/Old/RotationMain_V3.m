...
for i = 1:n_x
    for j = 1:n_y
        for k = 1:n_z
            ...
            A = Coord1.tool'*Coord2.tool;
            % Evaluation of the rotation matrix
            R = (A'*A)^0.5*A^-1;
            % Evaluation of the Euler angle
            
            angles = rotm2eul(R)*180/pi;
            angles2 = -angles;
            M = eul2rotm(angles2);
            
            Coord2.base = (M *Coord2.tool')';
            Matrix_dist(i,j,k) = abs(angles(2)+10)+abs(angles(1))+abs(angles(3));
...
% %%
figure()
scatter3(Coord1.tool(:,1),Coord1.tool(:,2),Coord1.tool(:,3) , 'filled', 'b');
hold on
scatter3(Coord2.tool(:,1),Coord2.tool(:,2),Coord2.tool(:,3) , 'filled', 'r');
% scatter3(Coord2.base(:,1),Coord2.base(:,2),Coord2.base(:,3) , 'filled', 'g');
axis equal
legend('coord1 tool','coord2 tool')
xlabel('x')
ylabel('y')
zlabel('z')
