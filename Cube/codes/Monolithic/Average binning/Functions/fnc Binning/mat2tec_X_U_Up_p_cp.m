function [] = mat2tec_X_U_Up_p_cp(X, U, U_std, p, cp, varargin)
    ...
    pmask = isnan(P); P(pmask) = 0; CP(pmask) = 0; 
	...
	savematrix = [x, y, z, u, v, w, umag, u_std, v_std, w_std, omega_x, omega_y, omega_z, OMEGA, Q, P, CP, No_Part, isValid];
	fid = fopen(filename, 'w');

	fprintf(fid, '%s\n', ['TITLE = "' filename '"']);
	fprintf(fid, '%s\n', 'VARIABLES = "X", "Y", "Z", "U [m/s]", "V [m/s]", "W [m/s]", "|U| [m/s]", "U_std [m/s]", "V_std [m/s]", "W_std [m/s]", "OmX [Hz]", "OmY [Hz]", "OmZ [Hz]", "|Om| [Hz]", "Q", "P", "CP", "No_Part", "isValid" ');
	fprintf(fid, '%s\n', ['ZONE T="' filename '", J=' num2str(lenx) ', I=' num2str(leny) ', K=' num2str(lenz)]);

	fprintf(fid, '%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %d %d\n', savematrix');
	fclose(fid);
end
