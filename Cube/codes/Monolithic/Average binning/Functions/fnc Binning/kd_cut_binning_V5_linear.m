function [U,varargout,RS] = kd_cut_binning_V5_linear(X, xm, um, bin_size, n_slices,min_np_found)
...
parfor i = 1 : n_slices
   ...
   for k = 1 : numel(xmesh)  % cycle on each centroid position
        ...
        if np_found_vec(k) >= max([min_np_found, 11])
            ...
            A  = [xm_sel, ones(size(xm_sel(:,1)))];
            um_sel_fit = zeros(size(xm_sel));
            for ii = 1:3
                c1 = A\um_sel(:,ii);
                u_binmean_vec(k, ii) = c1(4);
                
                % Calculate local mean velocities for 'better' prime
                % components
                um_sel_fit(:,ii) = c1(1) * xm_sel(:,1) + c1(2) * xm_sel(:,2) + c1(3) * xm_sel(:,3) + c1(4) ;
            end
