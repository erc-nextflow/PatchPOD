function [U,varargout,RS] = kd_cut_binning_V5_tophat(X, xm, um, bin_size, n_slices,min_np_found)
...
parfor i = 1 : n_slices
    ...
    for k = 1 : numel(xmesh)  % cycle on each centroid position
        ...
        if np_found_vec(k) >= min_np_found
            ...
            u_binmean_vec(k, :) = mean(um_sel);%sum(bsxfun(@times, um_sel, weights), 1) / sum(weights);
            u_binstd_vec(k, :)  = std(um_sel); %stdev_fun(um_sel, weights);
