...
% BINNING PARAMETERS %
%%%%%%%%%%%%%%%%%%%%%%
binsize         = 40;                                                       % Linear binsize in  [mm]
...
dataDir  = '..\Data';
naming    = 'Cone_';
n_cones = 1; %number of available cones
output_path = '.\Output';
...
for i = 1:n_cones
    %define output file
    Output_file = ['Cone_' num2str(i) '_' num2str(binsize) 'mm.mat'];
    Output_all = fullfile(output_path,Output_file);
    %load STB tracks
    Cone = load(fullfile(dataDir,[naming num2str(i) '.mat']));
    Tracks = Cone.Cone;
    %perform binning
    disp(['Binning cone ' num2str(i) ' out of ' num2str(n_cones)])
